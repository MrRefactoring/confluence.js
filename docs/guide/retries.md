...
try {
  ...
  if (error instanceof ApiError && error.status === 429) {
    // Atlassian sends Retry-After on rate limits — honour it, then re-queue.
...
## `withRetry` — rate limits, per call

The client's policy deliberately ignores `429`. When you *do* want a specific call to ride one out, `withRetry` wraps it:
...
It retries an `ApiError` carrying **429, 502, 503 or 504**, and nothing else — 401, 403, 404 and 500 are rethrown on the first attempt.

::: warning It is not a poller
`withRetry` only reacts to those four statuses. A plain `throw` inside the operation is not retried, so it cannot wait on eventually-consistent state:

```ts
// ❌ runs exactly once — the thrown Error is not an ApiError

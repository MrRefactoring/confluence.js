 ...
 *   - `v1Request` — authenticated v1 REST fetch for the label gap v2 lacks;
...
type Page = Awaited<ReturnType<ReturnType<typeof createCloudV2Client>['page']['createPage']>>;
...
describe('Confluence Cloud v2 — page.getLabelPages (live)', () => {
  ...
  beforeAll(async () => {
    ...
    const response = await v1Request<V1LabelResponse>(`content/${primaryPage.id}/label`, {
      method: 'POST',

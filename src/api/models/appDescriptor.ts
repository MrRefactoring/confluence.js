...
export interface AppDescriptor {
   ...
   *
   * Note: each app must have a unique baseUrl.
   * If you would like to serve multiple apps from the same host, consider adding a path prefix into the baseUrl.
   */
  baseUrl: string;
   ...
   *
   * The intention behind the API version is to allow vendors the ability to beta test a major revision to their
   * Connect app as a private version, and have a seamless transition for those beta customers (and existing customers)
   ...
   *
   * It's important to note that this approach allows vendors to create new versions manually,
   * despite the fact that in the common case, the versions are automatically created.
   ...
   * The description will be visible in the **Manage Apps** section of the administration console.
   * Provide meaningful and identifying information for the instance administrator.
   */
  description?: string;
   ...
   * the name used to distribute the app such as any listing in the [Atlassian Marketplace](https://marketplace.atlassian.com/).
   * Immutable records are created during the first installation of an app, one of which includes the name of the app.
   * It is therefore important that the name of the app is correct as it can not be changed.
   */
  name?: string;

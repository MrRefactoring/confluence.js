...
describe('Confluence Cloud v2 — blogPost.getLabelBlogPosts (live)', () => {
  it('returns the blog post tagged with a given label', async () => {
    ...
    try {
      const added = await rawRequest<{ results?: Array<{ id?: string; name?: string; prefix?: string }> }>(
    ...
    tracker.defer(async () => {
      await rawRequest(`content/${blogPost.id}/label/${labelName}`, { method: 'DELETE' }).catch(() => undefined);
    });
